...
class Rule {
    ...
    private $name;
    private $url;
    private $type = Null;  // cb (for callback) or file
    ...
    public function getDestType()
    {
        ...
        if(!is_callable($this->dest, False, $callable_name)) // only do this if the object is used ?
            if(file_exists($this->dest))
                ...
                throw new Error500('file '.$this->dest.' not found.');
        else
        {
...
class UrlDispatcher {
    ...
    *           array('news/' => 'func'),
    *           array('news-extra/', 'func', 'params' => array('id'=> 33)), // the order is important ! (pattern, callback, options)
    *       );
    *   
    *   return void
    **/
    ...
    * @return void
    */
    public function addPattern($pattern, $url, $dest, $name=False, array $extra_data=array())
    {
        $this->added_data[($name != False ? $name : crc32($pattern))] = array($pattern, $dest, $url, $extra_data);
    }
    
    /**
     * Returns the URL corresponding to a rule
     * 
     * \warning Do not call this method before the handle() method has benn
     *          called !
     * 
        ...
        foreach($this->added_data as $name => $data)
        {
            ...
            else
            {
                ...
                if($rule->getExtraData('action', 'exec') == 'read')
                    echo file_get_contents($rule->getDest());
